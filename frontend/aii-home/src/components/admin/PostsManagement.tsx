  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-white/40">
      ...
      <div className="mb-8 bg-blue-50/50 rounded-xl p-6 border border-blue-200">
        <h3 className="text-xl font-semibold text-blue-700 mb-4">{editingId ? '编辑文章' : '创建新文章'}</h3>
        <div className="space-y-4">
          ...
          <div className="flex space-x-3">
            {editingId ? (
              <>
                <button onClick={handleEditPost} className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">更新文章</button>
              ...
              <button onClick={handleCreatePost} className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">创建文章</button>
            )}
          </div>
        </div>
      </div>
      <div>
        ...
        {posts.length === 0 ? (
          <div className="text-center py-8 text-blue-600">暂无文章，创建你的第一篇文章吧！</div>
        ) : (
          <div className="space-y-4">
            {posts.map((post) => (
              <div key={post.id} className="bg-white rounded-xl p-4 border border-blue-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex justify-end space-x-3">
                  <button onClick={() => startEditing(post)} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-sm">编辑</button>
                </div>
              </div>
            ))}
          </div>
